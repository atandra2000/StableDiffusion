...
class PretrainedVAE(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        """Encode images in [-1, 1] to scaled latents."""
        dtype = torch.float16 if self.use_fp16 else x.dtype
        posterior = self.vae.encode(x.to(dtype=dtype)).latent_dist
        latents = posterior.mean if hasattr(posterior, "mean") else posterior.sample()
        return latents * self.scale_factor

    @torch.no_grad()
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        """Decode scaled latents back to pixel images in [-1, 1]."""
...
class DDPMScheduler:
    ...
    def get_velocity(self, x_0: torch.Tensor, noise: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """Velocity target used by v-prediction."""
        sqrt_alpha = self.sqrt_alphas_cumprod.to(x_0.device)[t].reshape(-1, 1, 1, 1)
        sqrt_sigma = self.sqrt_one_minus_alphas_cumprod.to(x_0.device)[t].reshape(-1, 1, 1, 1)
